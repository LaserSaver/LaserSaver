...
def main():
    ...
    initialContourList, listHierarchy, drawnEdges = FindContours.find_all_contours(inputImg)
    ...
    # drawing = np.zeros(inputImg.shape,np.uint8)
    # drawnContours = cv2.drawContours(drawing, finalContourList, -1, (255,255,255), 1)
    
    # cv2.imshow("Start", inputImg)
    # cv2.imshow("Edges", drawnEdges)
    # cv2.imshow("Contours", drawnContours)
    
    finalImage = FindContours.display_drawn_contours(inputImg,finalContourList)
